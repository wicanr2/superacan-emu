...
func (c *CPU) moveALongAddressIndirect(source, destination uint8) error {
...
func (c *CPU) moveAWordPostincrement(source, destination uint8) error {
	address := c.state.A[source]
	value, err := c.readWord(address, FCSupervisorData, PhaseDataRead)
	if err != nil {
		return err
	}
	c.state.A[source] = (address + 2) & addressMask
	c.state.A[destination] = uint32(int32(int16(value)))
	return c.prefetch()
}

func (c *CPU) addqWordDisplacement(register, quick uint8) error {
	stream := c.newInstructionStream()
	displacement, err := stream.nextWord()
	if err != nil {
		return err
...
func (c *CPU) moveAWordAddressIndirect(source, destination uint8) error {
	value, err := c.readWord(c.state.A[source], FCSupervisorData, PhaseDataRead)
	if err != nil {
		return err
	}
...
func (c *CPU) cmpiWordDisplacement(register uint8) error {
	stream := c.newInstructionStream()
	immediate, err := stream.nextWord()
	if err != nil {
		return err
...
func (c *CPU) addWordAbsoluteLongToData(destination uint8) error {
	stream := c.newInstructionStream()
	address, err := stream.nextLong()
	if err != nil {
		return err
	...
	result := c.add16(uint16(c.state.D[destination]), value)
	c.state.D[destination] = c.state.D[destination]&0xffff_0000 | uint32(result)
	return stream.finish()
}

func (c *CPU) orLongAddressIndirectToData(source, destination uint8) error {
	value, err := c.readLong(c.state.A[source], FCSupervisorData)
	if err != nil {
		return err
	}
...
func (c *CPU) divuWordAbsoluteLong(destination uint8) error {
	stream := c.newInstructionStream()
	address, err := stream.nextLong()
	if err != nil {
		return err
	}
	divisor, err := c.readWord(address, FCSupervisorData, PhaseDataRead)
	if err != nil {
		return err
	}
	if divisor == 0 {
...
func (c *CPU) divuWordDisplacement(source, destination uint8) error {
	stream := c.newInstructionStream()
	displacement, err := stream.nextWord()
	if err != nil {
		return err
...
func (c *CPU) addWordAddressToData(source, destination uint8) error {
	result := c.add16(uint16(c.state.D[destination]), uint16(c.state.A[source]))
	c.state.D[destination] = c.state.D[destination]&0xffff_0000 | uint32(result)
	return c.prefetch()
}

func (c *CPU) moveWordAbsoluteLongToAbsoluteLong() error {
	stream := c.newInstructionStream()
	...
	value, err := c.readWord(source, FCSupervisorData, PhaseDataRead)
	if err != nil {
		return err
	}
	c.setNZ16(value)
	if err := c.writeWord(destination, value, FCSupervisorData); err != nil {
		return err
	}
	return stream.finish()
}

func (c *CPU) movemWordPostincrementToRegisters(addressRegister uint8) error {
	stream := c.newInstructionStream()
	mask, err := stream.nextWord()
	if err != nil {
		return err
	...
	for register := uint8(0); register < 16; register++ {
		...
		c.setRegisterValue(register, uint32(int32(int16(value))))
		address = (address + 2) & addressMask
	}
	c.state.A[addressRegister] = address
	return stream.finish()
}

func (c *CPU) orWordAbsoluteLongToData(destination uint8) error {
	stream := c.newInstructionStream()
	address, err := stream.nextLong()
	if err != nil {
		return err
...
func (c *CPU) moveALongData(source, destination uint8) error {
...
func (c *CPU) btstImmediateDisplacement(register uint8) error {
	stream := c.newInstructionStream()
	immediate, err := stream.nextWord()
	if err != nil {
		return err
	...
	bit := uint8(immediate & 7)
	c.state.SR &^= flagZero
	if value&(uint8(1)<<bit) == 0 {
		c.state.SR |= flagZero
	}
	return stream.finish()
}

func (c *CPU) moveLongPostincrementToDisplacement(source, destination uint8) error {
	stream := c.newInstructionStream()
	displacement, err := stream.nextWord()
	if err != nil {
		return err
...
func (c *CPU) addWordDataToDisplacement(source, destination uint8) error {
	stream := c.newInstructionStream()
	displacement, err := stream.nextWord()
	if err != nil {
		return err
...
func (c *CPU) moveLongAddressIndirectToDisplacement(source, destination uint8) error {
	stream := c.newInstructionStream()
	displacement, err := stream.nextWord()
	if err != nil {
		return err
