...
func TestCLRLongAbsoluteLongReadModifyWrite(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x42b9, 0x0402: 0x0001, 0x0404: 0x2000,
		0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012000: 0x8000, 0x012002: 1,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	if state := cpu.State(); result.Cycles != 28 || state.SR&0x1f != flagExtend|flagZero {
		t.Fatalf("cycles=%d SR=$%04X", result.Cycles, state.SR)
	}
	want := []wordWrite{{address: 0x012000, value: 0}, {address: 0x012002, value: 0}}
	if !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("writes=%+v, want %+v", bus.writes, want)
	}
}

func TestLSRLongImmediate(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0xe88f, 0x0402: 0x4e71, 0x0404: 0x4e71,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[7] = 0x8000_000f
	cpu.state.SR |= flagZero | flagOverflow
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
...
func TestMOVEALongIndexedPreservesFlags(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x2c70, 0x0402: 0x1004, 0x0404: 0x4e71, 0x0406: 0x4e71,
		0x0108: 0x89ab, 0x010a: 0xcdef,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[0] = 0x0100
	cpu.state.D[1] = 4
	cpu.state.SR |= flagCarry | flagZero
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 18 || state.A[6] != 0x89ab_cdef || state.SR&0x1f != flagCarry|flagZero {
		t.Fatalf("cycles=%d A6=$%08X SR=$%04X", result.Cycles, state.A[6], state.SR)
	}
}

func TestANDIWordAbsoluteLongReadModifyWrite(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x0279, 0x0402: 0x0ff0, 0x0404: 0x0001, 0x0406: 0x2000,
		0x0408: 0x4e71, 0x040a: 0x4e71, 0x012000: 0x8ff5,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	if state := cpu.State(); result.Cycles != 28 || state.SR&0x1f != flagExtend {
		t.Fatalf("cycles=%d SR=$%04X", result.Cycles, state.SR)
	}
	want := []wordWrite{{address: 0x012000, value: 0x0ff0}}
	if !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("writes=%+v, want %+v", bus.writes, want)
	}
}

func TestTSTLongAddressIndirect(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x4a96, 0x0402: 0x4e71, 0x0404: 0x4e71,
		0x012000: 0x8000, 0x012002: 0x0001,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
...
func TestMOVELongAbsoluteLongToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x2039, 0x0402: 0x0001, 0x0404: 0x2000,
		0x0406: 0x4e71, 0x0408: 0x4e71, 0x012000: 0x1234, 0x012002: 0x5678,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	if state := cpu.State(); result.Cycles != 20 || state.D[0] != 0x1234_5678 || state.SR&0x1f != 0 {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestMOVELongRegisterSourcesToData(t *testing.T) {
	for _, tc := range []struct {
		name   string
		opcode uint16
		value  uint32
	}{
		{name: "data", opcode: 0x2e01, value: 0x8000_0001},
		{name: "address", opcode: 0x200e, value: 0x8000_0001},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: tc.opcode, 0x0402: 0x4e71, 0x0404: 0x4e71})
			if err := cpu.Reset(); err != nil {
				t.Fatal(err)
			}
			cpu.state.D[1] = tc.value
			cpu.state.A[6] = tc.value
			result, err := cpu.Step()
			if err != nil {
				t.Fatal(err)
			}
			if state := cpu.State(); result.Cycles != 4 || state.D[uint8(tc.opcode>>9)&7] != tc.value || state.SR&0x1f != flagNegative {
				t.Fatalf("cycles=%d state=%+v", result.Cycles, state)
			}
		})
	}
}

func TestMOVELongAddressToAddressIndirect(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{0x0400: 0x2c88, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[0] = 0x89ab_cdef
	cpu.state.A[6] = 0x012000
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
...
func TestADDQLongData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x5280, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	if state := cpu.State(); result.Cycles != 8 || state.D[0] != 0x8000_0000 || state.SR&0x1f != flagNegative|flagOverflow {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestMOVELongAddressToDisplacement(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{0x0400: 0x2d49, 0x0402: 0xfffc, 0x0404: 0x4e71, 0x0406: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[1] = 0x1234_5678
	cpu.state.A[6] = 0x012004
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	want := []wordWrite{{address: 0x012000, value: 0x1234}, {address: 0x012002, value: 0x5678}}
	if state := cpu.State(); result.Cycles != 16 || state.SR&0x1f != 0 || !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("cycles=%d SR=$%04X writes=%+v", result.Cycles, state.SR, bus.writes)
	}
}

func TestDIVUWordImmediate(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x80fc, 0x0402: 7, 0x0404: 0x4e71, 0x0406: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[0] = 100
	...
	if state := cpu.State(); result.Cycles != 144 || state.D[0] != 0x0002_000e || state.SR&0x1f != 0 {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestCLRLongDisplacementReadModifyWrite(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x42ae, 0x0402: 4, 0x0404: 0x4e71, 0x0406: 0x4e71,
	...
	want := []wordWrite{{address: 0x012004, value: 0}, {address: 0x012006, value: 0}}
	if state := cpu.State(); result.Cycles != 24 || state.SR&0x1f != flagZero || !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("cycles=%d SR=$%04X writes=%+v", result.Cycles, state.SR, bus.writes)
	}
}

func TestJSRAddressIndirect(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x4e96, 0x0402: 0x4e71, 0x0404: 0x4e71,
		0x0800: 0x4e71, 0x0802: 0x4e71,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[6] = 0x0800
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	want := []wordWrite{{address: 0x000ffc, value: 0}, {address: 0x000ffe, value: 0x0402}}
	if state := cpu.State(); result.Cycles != 16 || state.PC != 0x0800 || state.A[7] != 0x000ffc || !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("cycles=%d PC=$%06X A7=$%06X writes=%+v", result.Cycles, state.PC, state.A[7], bus.writes)
	}
}

func TestADDWordDataToPostincrementReadModifyWrite(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{0x0400: 0xd15a, 0x0402: 0x4e71, 0x0404: 0x4e71, 0x012000: 0x7fff})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[0] = 1
	...
	want := []wordWrite{{address: 0x012000, value: 0x8000}}
	if state := cpu.State(); result.Cycles != 12 || state.A[2] != 0x012002 || state.SR&0x1f != flagNegative|flagOverflow || !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("cycles=%d A2=$%06X SR=$%04X writes=%+v", result.Cycles, state.A[2], state.SR, bus.writes)
	}
}

func TestMOVEAWordAbsoluteLongSignExtends(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x3479, 0x0402: 0x0001, 0x0404: 0x2000, 0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012000: 0xff80,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	...
	if state := cpu.State(); result.Cycles != 16 || state.A[2] != 0xffff_ff80 || state.SR&0x1f != flagZero|flagCarry {
		t.Fatalf("cycles=%d A2=$%08X SR=$%04X", result.Cycles, state.A[2], state.SR)
	}
}

func TestMOVELongDataToPostincrement(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{0x0400: 0x20c1, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	want := []wordWrite{{address: 0x012000, value: 0x89ab}, {address: 0x012002, value: 0xcdef}}
	if state := cpu.State(); result.Cycles != 12 || state.A[0] != 0x012004 || state.SR&0x1f != flagNegative || !reflect.DeepEqual(bus.writes, want) {
		t.Fatalf("cycles=%d A0=$%06X SR=$%04X writes=%+v", result.Cycles, state.A[0], state.SR, bus.writes)
	}
}

func TestMOVEWordAddressToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x320a, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[2] = 0x1234_8000
	cpu.state.D[1] = 0xabcd_ffff
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 4 || state.D[1] != 0xabcd_8000 || state.SR&0x1f != flagNegative {
		t.Fatalf("cycles=%d D1=$%08X SR=$%04X", result.Cycles, state.D[1], state.SR)
	}
}

func TestMOVEALongAddressIndirectPreservesFlags(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x2056, 0x0402: 0x4e71, 0x0404: 0x4e71,
		0x012000: 0x89ab, 0x012002: 0xcdef,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	...
	if state := cpu.State(); result.Cycles != 12 || state.A[0] != 0x89ab_cdef || state.SR&0x1f != flagZero|flagCarry {
		t.Fatalf("cycles=%d A0=$%08X SR=$%04X", result.Cycles, state.A[0], state.SR)
	}
}

func TestMOVEAWordPostincrementSignExtends(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x3658, 0x0402: 0x4e71, 0x0404: 0x4e71, 0x012000: 0xff80})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[0] = 0x012000
	...
	if state := cpu.State(); result.Cycles != 8 || state.A[0] != 0x012002 || state.A[3] != 0xffff_ff80 || state.SR&0x1f != flagZero|flagCarry {
		t.Fatalf("cycles=%d A0=$%06X A3=$%08X SR=$%04X", result.Cycles, state.A[0], state.A[3], state.SR)
	}
}

func TestADDQWordDisplacementReadModifyWrite(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{0x0400: 0x526e, 0x0402: 4, 0x0404: 0x4e71, 0x0406: 0x4e71, 0x012004: 0x7fff})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[6] = 0x012000
...
func TestMOVEAWordAddressIndirectSignExtends(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x3851, 0x0402: 0x4e71, 0x0404: 0x4e71, 0x012000: 0xff80})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	if state := cpu.State(); result.Cycles != 8 || state.A[4] != 0xffff_ff80 || state.SR&0x1f != flagZero|flagCarry {
		t.Fatalf("cycles=%d A4=$%08X SR=$%04X", result.Cycles, state.A[4], state.SR)
	}
}

func TestCMPIWordDisplacement(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x0c6e, 0x0402: 0x1234, 0x0404: 4, 0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012004: 0x1234,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	...
	if state := cpu.State(); result.Cycles != 16 || state.SR&0x1f != flagZero {
		t.Fatalf("cycles=%d SR=$%04X", result.Cycles, state.SR)
	}
}

func TestADDWordAbsoluteLongToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0xde79, 0x0402: 0x0001, 0x0404: 0x2000, 0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012000: 1,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[7] = 0xabcd_7fff
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 16 || state.D[7] != 0xabcd_8000 || state.SR&0x1f != flagNegative|flagOverflow {
		t.Fatalf("cycles=%d D7=$%08X SR=$%04X", result.Cycles, state.D[7], state.SR)
	}
}

func TestORLongAddressIndirectToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x8091, 0x0402: 0x4e71, 0x0404: 0x4e71, 0x012000: 0x8000, 0x012002: 1})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[1] = 0x012000
	...
	if state := cpu.State(); result.Cycles != 12 || state.D[0] != 0x8000_0003 || state.SR&0x1f != flagNegative {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestDIVUWordAbsoluteLong(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x8ef9, 0x0402: 0x0001, 0x0404: 0x2000, 0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012000: 7,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	...
	if state := cpu.State(); result.Cycles != 152 || state.D[7] != 0x0002_000e || state.SR&0x1f != 0 {
		t.Fatalf("cycles=%d D7=$%08X SR=$%04X", result.Cycles, state.D[7], state.SR)
	}
}
...
func TestDIVUWordDisplacement(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x80ea, 0x0402: 4, 0x0404: 0x4e71, 0x0406: 0x4e71, 0x012004: 7})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[2] = 0x012000
	...
	if state := cpu.State(); result.Cycles != 148 || state.D[0] != 0x0002_000e || state.SR&0x1f != 0 {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestADDWordAddressToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0xd24e, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.A[6] = 1
	cpu.state.D[1] = 0xabcd_7fff
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 4 || state.D[1] != 0xabcd_8000 || state.SR&0x1f != flagNegative|flagOverflow {
		t.Fatalf("cycles=%d D1=$%08X SR=$%04X", result.Cycles, state.D[1], state.SR)
	}
}

func TestMOVEWordAbsoluteLongToAbsoluteLong(t *testing.T) {
	cpu, _, bus := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x33f9, 0x0402: 0x0001, 0x0404: 0x1000, 0x0406: 0x0001, 0x0408: 0x2000,
		0x040a: 0x4e71, 0x040c: 0x4e71, 0x011000: 0x8001,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
...
func TestMOVEMWordPostincrementToRegisters(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x4c9f, 0x0402: 0x8081, 0x0404: 0x4e71, 0x0406: 0x4e71,
		0x001000: 0xff80, 0x001002: 0x1234, 0x001004: 0x5678,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	...
	state := cpu.State()
	if result.Cycles != 20 || state.D[0] != 0xffff_ff80 || state.D[7] != 0x0000_1234 || state.A[7] != 0x001006 {
		t.Fatalf("cycles=%d D0=$%08X D7=$%08X A7=$%06X", result.Cycles, state.D[0], state.D[7], state.A[7])
	}
}

func TestORWordAbsoluteLongToData(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{
		0x0400: 0x8079, 0x0402: 0x0001, 0x0404: 0x2000, 0x0406: 0x4e71, 0x0408: 0x4e71,
		0x012000: 0x8001,
	})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[0] = 0xabcd_0002
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 16 || state.D[0] != 0xabcd_8003 || state.SR&0x1f != flagNegative {
		t.Fatalf("cycles=%d D0=$%08X SR=$%04X", result.Cycles, state.D[0], state.SR)
	}
}

func TestMOVEALongDataPreservesFlags(t *testing.T) {
	cpu, _, _ := newInstructionCPU(map[uint32]uint16{0x0400: 0x2c43, 0x0402: 0x4e71, 0x0404: 0x4e71})
	if err := cpu.Reset(); err != nil {
		t.Fatal(err)
	}
	cpu.state.D[3] = 0x89ab_cdef
	cpu.state.SR |= flagZero | flagCarry
	result, err := cpu.Step()
	if err != nil {
		t.Fatal(err)
	}
	if state := cpu.State(); result.Cycles != 4 || state.A[6] != 0x89ab_cdef || state.SR&0x1f != flagZero|flagCarry {
		t.Fatalf("cycles=%d A6=$%08X SR=$%04X", result.Cycles, state.A[6], state.SR)
	}
}
