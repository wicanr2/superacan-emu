...
事實已經蒐集完畢，缺的是決定本身。每一項在拍板前都不動程式。

- [ ] **A1 cgo 邊界對三個發行平台的適用範圍。** 支持原本「全 binary 禁 cgo」的兩個
  目標（js/wasm、windows）都不在發行範圍內了。實測：`cmd/acan` 在 linux／darwin／
  android 的 `CGO_ENABLED=0` 建置全部失敗，各自原因不同；`oto/v3` 在 darwin 走 purego
  可無 cgo 建置，所以 macOS 缺的只有視窗與繪圖層。三個選項與代價見
  `docs/platform-targets.md`。建議把禁令縮到模擬核心，平台層按平台決定。
- [ ] **A2 字型走嵌入或讀系統。** `basicfont` 只涵蓋可列印 ASCII，法文與西班牙文的
  重音字元和中文擋在同一個閘門。`bitmapfont/v4` 的六份來源授權都允許散布，代價是
  2.5 MB 與六份授權文字；讀系統字型省空間但 Linux 桌面不保證有中文字型。
  見 `docs/ui-font.md`。介面語言的範圍隨這一項決定。
- [ ] **A3 擷取做到哪一級。** MP4 不做（純 Go 沒有 H.264／AAC 編碼器）。候選是 GIF
  （stdlib、對得上硬體 256 色、無聲、30 fps）、外部 `--capture-sink`（桌面可得有聲
  MP4，自備 ffmpeg）、`.acanraw` 無損中間格式。見 `docs/ui-design.md` §4.7。
- [ ] **A4 金手指是否進第一個發行版。** 取捨是「發行版帶作弊工具」對上「發行版的
  執行結果可作硬體證據」。設計已規定啟用時畫面常駐標記且該工作階段的雜湊不得作證據。
- [ ] **A5 桌面 Esc 由「離開」改為「開啟選單」。** 這會改變現行 X11 前端的行為，
  要在發行說明標出；不改則 Android 返回鍵語意與桌面不一致。
...
- [ ] **P6** 金手指：搜尋、清單、`ACANCHT1` 讀寫與 `BCAN_CHT_1` 匯入。前置 A4。
- [ ] **P7** 擷取：截圖與依 A3 決定的錄影方案。
- [ ] **P8** 多語言。前置 A2。
