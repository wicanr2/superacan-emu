...
規劃見 [`docs/ui-plan.md`](docs/ui-plan.md)。五個階段各自的驗收條件寫在該文件；
每一階段都要維持九款卡帶的 1200-frame 指令數與 framebuffer SHA-256 不變。

- [ ] 階段一：`ui` 套件骨架、覆蓋選單、存讀檔槽。
- [ ] 階段二：無卡帶啟動畫面與卡帶瀏覽器。
- [ ] 階段三：按鍵設定與 JSON 設定檔（未知欄位忽略）。
- [ ] 階段四：影音設定與診斷面板；一併收尾「有界、可篩選的除錯介面」。
- [ ] 階段五：繁體中文與語言切換。動工前先確認 CJK 字型的散布授權。
