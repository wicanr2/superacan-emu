...
實測**五個選單**（Wine 內開啟每一個選單截圖，載入卡帶前後都一樣）：

| 選單 | 項目 |
|---|---|
| 檔案 (&F) | 開啟 ROM…（Ctrl+O）／重新啟動遊戲 (&R)／循環存檔槽（0–9）／離開 (&X) |
| 顯示 (&V) | 整數縮放 (&S) ▸／全螢幕（Alt+Enter）／以 4:3 顯示（預設）／視訊濾鏡 (&V) ▸／動態平滑（減少 LCD 拖影）／全速時靜音／隱藏熱鍵操作訊息（錯誤仍顯示）／錄影格式 ▸ |
| 輸入 (&I) | P1 鍵盤／手把…／P2 鍵盤／手把…／熱鍵設定… |
| 語言 (&L) | English／Français／Español／繁體中文／简体中文 |
| 說明 (&H) | 關於 Bcan／效能診斷：開始／停止並儲存 |

`▸` 是有子選單的項目。截圖收在
[`docs/screenshots/bcan/`](screenshots/bcan/)（未載入卡帶，不含遊戲畫面）。

**金手指不在選單列上。** 字串表裡有完整的金手指介面文字（記憶體搜尋、清單、
鎖定全部），`Bcan.ini` 也有 `hotkey_lock_all_cheats`，所以功能確實存在；
但選單列在「未載入卡帶」與「已載入卡帶」兩種狀態下都只有上面五項。它從哪裡開啟
還沒查證——可能是熱鍵、可能有另一個開關。在查證之前不要當成選單列的一項。
